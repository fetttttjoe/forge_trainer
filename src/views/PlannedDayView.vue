...
<template>
  <div class="flex h-[100dvh] flex-col bg-bg text-ink">
    ...
    <div class="flex-1 overflow-y-auto px-5 pb-5 pt-[6px]">
      ...
      <div
        class="mt-[10px] inline-flex items-center gap-[6px] rounded-[9px] bg-[color-mix(in_srgb,var(--accent)_12%,transparent)] px-[11px] py-[5px] text-[11px] font-extrabold uppercase tracking-[0.03em] text-accent"
      >
        <svg
          width="12"
          height="12"
      ...
      <div class="mt-4 flex flex-col gap-2">
        <div
          v-for="e in list"
          :key="e.id"
          class="flex items-center gap-3 rounded-[14px] border border-line bg-surface px-[14px] py-[12px] shadow-card"
        >
          <div
            class="flex h-[38px] w-[38px] shrink-0 items-center justify-center rounded-[10px] bg-surface-2 text-[16px]"
          >
            {{ e.icon }}
          </div>
