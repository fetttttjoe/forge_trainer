...
const improveLabel = computed(() => {
  ...
  const topW = (h: (typeof eh)[number]) =>
    h.entries
      .find((x) => x.exId === c.exId)!
      .sets.filter((x) => x.done)
      .reduce((m, x) => Math.max(m, x.weight || 0), 0)
  const f = topW(eh[0])
  const l = topW(eh[eh.length - 1])
  if (f > 0 && l > 0 && l !== f)
    return (l > f ? '▲ +' : '▼ −') + w(Math.abs(l - f)) + ' ' + unit.value + ' since you started'
  return ''
})
...
const hasTimer = computed(
  () => timer.value.timeLeft > 0 || (timer.value.mode === TimerMode.Rest && timer.value.timerOn),
)
...
<template>
  <div v-if="cur" class="flex h-[100dvh] flex-col bg-bg text-ink">
    ...
    <div class="flex-1 overflow-y-auto px-[18px] pb-5 pt-[6px]">
      ...
      <div
        v-if="isInterval && psets.length"
        class="mt-[9px] inline-flex items-center gap-[7px] rounded-[9px] bg-surface-2 px-[11px] py-[6px] text-[12px] font-bold text-ink-2"
      >
        <svg
          width="13"
          height="13"
      ...
      <div v-if="isInterval" class="mt-[18px] flex flex-col items-center">
        ...
        <div class="mt-[18px] flex w-full gap-[10px]">
          <button
            type="button"
            class="flex-1 cursor-pointer rounded-[14px] border-none bg-accent p-[15px] text-[15px] font-bold text-white"
            @click="workout.startIntervalTimer()"
          >
            Start set
          </button>
          <button
            type="button"
            class="w-[64px] cursor-pointer rounded-[14px] border border-line bg-surface text-[13px] font-bold text-ink"
            @click="workout.pauseTimer()"
          >
            {{ timer.timerOn ? 'Pause' : 'Resume' }}
          </button>
        </div>
      </div>
      ...
      <div v-else>
        <div
          v-if="psets.length"
          class="mt-4 flex items-center justify-between gap-3 rounded-[14px] border border-line bg-surface px-[15px] py-[12px] shadow-card"
        >
          <div class="min-w-0">
            <div class="text-[11px] font-bold uppercase tracking-[0.04em] text-ink-3">
              Last time · {{ cur.prev ? relTime(cur.prev.date) : '' }}
            </div>
        ...
        <div
          class="mt-4 grid grid-cols-[34px_minmax(0,1fr)_minmax(0,1fr)_46px] gap-2 px-[11px] pb-[6px] text-[11px] font-bold uppercase tracking-[0.04em] text-ink-3"
        >
          <span class="text-center">Set</span><span class="text-center">Weight · {{ unit }}</span
          ><span class="text-center">Reps</span><span />
        </div>
        ...
        <div v-if="hasTimer" class="mt-4 flex items-center gap-[14px] rounded-[16px] bg-ink px-4 py-[14px] text-bg">
          <div class="flex-1">
            ...
            <div class="mt-px text-[32px] font-extrabold leading-none tabular-nums tracking-[-0.02em]">
              {{ timeText }}
            </div>
          </div>
          <button
            type="button"
            class="cursor-pointer rounded-[11px] border-none bg-[color-mix(in_srgb,var(--bg)_16%,transparent)] px-3 py-[9px] text-[13px] font-bold text-bg"
            @click="workout.addTimer()"
          >
            +15s
          </button>
          <button
            type="button"
            class="flex h-[42px] w-[42px] cursor-pointer items-center justify-center rounded-[11px] border-none bg-[color-mix(in_srgb,var(--bg)_16%,transparent)] text-bg"
            @click="workout.pauseTimer()"
          >
            <svg v-if="timer.timerOn" width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
    ...
    <div class="flex shrink-0 gap-[10px] border-t border-line bg-bg px-[18px] pb-[26px] pt-3">
      <button
        type="button"
        class="flex w-[52px] shrink-0 cursor-pointer items-center justify-center rounded-[14px] border border-line bg-surface text-ink"
        @click="workout.prevEx()"
      >
        <svg
          width="20"
          height="20"
