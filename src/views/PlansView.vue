...
<template>
  <div>
    ...
    <div v-for="p in cards" :key="p.id" class="mb-[11px] rounded-[18px] border border-line bg-surface p-4 shadow-card">
      <button
        type="button"
        class="block w-full cursor-pointer border-none bg-transparent p-0 text-left text-ink"
        @click="router.push(paths.plan(p.id))"
      >
        ...
        <div class="mt-3 flex flex-wrap gap-[6px]">
          <span
            v-for="t in p.tags"
            :key="t"
            class="rounded-full bg-surface-2 px-[10px] py-1 text-[11px] font-semibold text-ink-2"
            >{{ t }}</span
          >
        </div>
      </button>
